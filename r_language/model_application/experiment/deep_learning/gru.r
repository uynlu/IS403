...
GRU_model <- function(input_size, hidden_size = 50, num_layers = 3, dropout_prob = 0.3, n_steps = 1) {
    model <- keras_model_sequential() %>%
    layer_input(shape = c(NULL, input_size)) %>%
    layer_gru(units = hidden_size, num_layers = num_layers, return_sequences = TRUE) %>%
...
run_pipeline <- function(train_path, validation_path, test_path, look_back, n_steps) {
    ...
    gru <- GRU_model(input_size = 9, hidden_size = 50, num_layers = 3, dropout_prob = 0.3, n_steps = 1)
    print(summary(gru))
